...
6. 오른쪽 패널에서는 현재 경로(Primary/Fallback), 활성 에이전트, tool 호출 결과를 확인할 수 있습니다.
...
- 상단 `대기 중 / PRIMARY PATH / RT-2 FALLBACK`: 현재 세션이 기존 workflow 중심인지, RT-2 fallback 문맥으로 시작했는지 보여줍니다.
...
- 프리셋: `S1 단순 FAQ`
- 기대 흐름: Workflow 시뮬레이션은 `처리됨`, 상단 경로는 Primary로 유지되며 첫 greeting도 일반 배송/주문 문의 톤으로 시작됩니다.
- 예시 발화: `배송 얼마나 걸려요?`
...
- 프리셋: `S2 복합 의도`
- 기대 흐름: Workflow 시뮬레이션은 `미처리`, 전환 배너와 RT-2 fallback 경로가 표시되며 첫 greeting에서 주문 변경/배송지 수정/쿠폰 요청을 이미 인지한 듯 시작합니다.
...
- 프리셋: `S3 불만 고객`
- 기대 흐름: 배송 지연과 불만 고객 문맥이 주입된 상태로 fallback 세션이 시작되며 첫 greeting에서 사과/공감 표현이 먼저 나옵니다.
